...
// --- Map Enhancements Injected Below ---
...
function canonicalStateName(name) {
  ...
  if (n.includes("andhra") && n.includes("pradesh")) return "Andhra Pradesh";
  if (n === "telangana") return "Telangana";
  if (["jammu and kashmir", "jammu & kashmir"].includes(n)) return "Jammu and Kashmir";
  if (["odisha", "orissa"].includes(n)) return "Odisha";
  if (["dadra and nagar haveli", "daman and diu", "dadra & nagar haveli"].includes(n))
    return "Dādra and Nagar Haveli and Damān and Diu";
  if (n.startsWith("pan india") || n.includes("other centralized funds"))
    return "PAN India";
  if (n.includes("not mentioned") || n.startsWith("nec")) return "Unspecified geography";
  return name.trim();
}

// Dummy example for now – you should connect this to your actual filtered dataset
function getStateCSRValue(stateName) {
  const mockValues = {
    "Andhra Pradesh": 123.4,
    "Telangana": 87.6,
    "Arunachal Pradesh": 45.2,
    "Odisha": 132.1
  };
  return mockValues[stateName] || "0.0";
}
...
function highlightMapStates(selectedStates) {
  ...
  svgContainer.querySelectorAll('path, g').forEach(elem => elem.classList.remove('state-selected'));
  svgContainer.querySelectorAll('.state-label').forEach(el => el.remove());

  if (selectedStates.includes("PAN India")) {
    svgContainer.querySelectorAll('path').forEach(p => p.classList.add('state-selected'));
    return;
  }

  selectedStates.forEach(state => {
    const el = svgContainer.querySelector(`[id='${state}'], [data-state='${state}']`);
    if (el) {
      ...
      const value = getStateCSRValue(state);
      ...
      label.textContent = `${state}: ₹${value} Cr`;
...
function enhanceMapUI() {
  const mapPanel = document.querySelector("#map .map-section");
  if (!mapPanel) return;
  ...
  header.innerHTML = `
    ...
    <p class="data-description">Spending, Projects, and Company counts by state for selected filters</p>
  `;
  mapPanel.prepend(header);

  const filtersDiv = document.createElement("div");
  filtersDiv.id = "mapFiltersSummary";
  filtersDiv.style.margin = "0.5rem 0";
  filtersDiv.style.fontSize = "0.9rem";
  filtersDiv.style.color = "#666";
  filtersDiv.textContent = getSelectedFiltersSummary?.() || "";
  header.appendChild(filtersDiv);

  const exportBtn = document.createElement("button");
  exportBtn.className = "btn btn--outline btn--sm";
  exportBtn.textContent = "📥 Export Map as PNG";
